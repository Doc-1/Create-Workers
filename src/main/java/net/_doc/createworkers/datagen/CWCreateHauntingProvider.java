...
public class CWCreateHauntingProvider extends ProcessingRecipeGen {
    ...
    GeneratedRecipe BRASS_BELL = convert(() -> Ingredient.of(CWBlocks.TEMP_BLOCK.get()), AllBlocks.ANDESITE_CASING::get);
    ...
    public GeneratedRecipe convert(Supplier<Ingredient> input, Supplier<ItemLike> result) {
        return create(CreateWorkers.asResource(RegisteredObjects.getKeyOrThrow(result.get().asItem()).getPath()), p -> p.withItemIngredients(input.get()).output(result.get()));
    }
